...
import {
  ArrowRight, Pencil, Mic, Inbox, Mail,
...
export default function DashboardPage() {
  const username = (() => { try { return JSON.parse(localStorage.getItem('user') || '{}').name || 'there' } catch { return 'there' } })()
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-12">
          {SCENARIOS.map(s => (
            <div key={s.key} className="scenario-card cursor-pointer group overflow-hidden"
                 onClick={() => navigate(`/courses/ai/${s.key}`)}>
