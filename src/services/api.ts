...
// Base URL points to /prod — all functions append /students themselves
const API_BASE = "https://corsproxy.io/?https://d25zzadgyf.execute-api.us-east-1.amazonaws.com/prod";
...
export async function getAllStudents(): Promise<any> {
  try {
    ...
    const promises = recordIds.map(id =>
      fetch(`${API_BASE}/students?record_id=${id}`, {
